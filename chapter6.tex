...
the same type property and, consequently, the same set of fields. The
interdependence between a \texttt{Mapper} and its assigned data type is
accentuated even further by the \texttt{getFieldDescriptor()} and
...
by the type checking mechanism embedded in various \texttt{Script}
instructions, where it is employed for checking wether a specific operation can
be safely performed or not (see section~\ref{sec:components.script} for further
details).
...
the \texttt{marshal()} and \texttt{unmarshal()} methods. As can be seen by
analyzing their respective signatures, these two methods can be used to convert
\texttt{Message} objects into a \texttt{Payload} and vice-versa. This
additional functionality introduces another aspect of the \texttt{Mapper}
interface, namely its ability to manage different formats of binary data.
