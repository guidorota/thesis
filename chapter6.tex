...
execution. The current status of each \texttt{Scrip} execution instance is
stored inside a private \texttt{ExecutionContext} Java object, which contains
the following elements:

\begin{itemize}

    \item A reference to the current instruction (the Program Counter);

    \item The current value of all declared variables;

    \item The record staging area;

    \item A list of emitted records (i.e., records to be returned when the
        \texttt{stop} instruction is encountered). 

\end{itemize}

The \texttt{ExecutionContext} object isolates threads running concurrently, as
it is the only object that a \texttt{Script} is allowed to alter while it's
being executed. This design ensures that all changes a \texttt{Script} makes to
its environment remain private, and that the its status cannot be altered by
other misbehaving or poorly designed routines. Furthermore, it allows PerLa
\texttt{Scripts} to be interrupted and resumed at will, as 
