...
The data encapsulated in a \texttt{Payload} object is accessed with byte
granularity. This level of abstraction is ideal for the implemention of an I/O
access layer, whose sole concern is the transmission of information between two
endpoints, but is not well suited to other forms of data management.
Information is usually organized following a well defined structure, whose
characteristics cannot be leveraged with the \texttt{Payload} interface; when
data can only be read byte after byte, even the simplest arithmetic expression
may become unwieldy and unnecessarily complex to perform.

The \texttt{Message} interface provides an additional level of abstraction that
allows us to overcome the aforementioned limitations. A \texttt{Message} is a
structured container of information; data can be accessed by field name, and
each field is 
