...
that cannot be employed where a \texttt{Payload} is expected, as it doesn't
provide any method for accessing its contents as a simple sequence of bytes;
therefore, \texttt{Message}s can't be used directly to perform any kind of I/O
operation.
...
\texttt{Message}s and vice-versa. This process is defined as
...
\texttt{Payload} is decoded into a \texttt{Message} object. The additional
level of decoupling provided by the \texttt{Mapper} interface is a fundamental
design feature that is exploited to provide support for different data formats.

Data transmitted between the Middleware and an endpoint device may be
structured using a plethora of formats, like JSON and XML (REST APIs), C-like
data structures (low-power or embedded devices), serialized Java classes (Java
RMI APIs) or proprietary data encoding mechanism.
