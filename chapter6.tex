...
The interaction with a \texttt{Mapper} usually begins with a call to the
...
the same data type property and, consequently, the same set of fields.
...
relationship between \texttt{Mapper}s and \texttt{Message} types is epitomized
by the \texttt{getFieldDescriptors()} method, which returns a complete
description of every field included in a specific \texttt{Message} type. This
method holds a pivotal role in the implementation of the Middleware's type
safety mechanism, where the possibility to introspect the internal structure of
a \texttt{Message} object is heavily exploited for checking wether a specific
operation can be safely performed or not.
...
This structural gap is bridged by the \texttt{marshal()} and
\texttt{unmarshal()} methods. As can be seen by analyzing their respective
signatures, these two methods can be used to convert \texttt{Message} objects
into a \texttt{Payload} and vice-versa. This additional functionality
introduces another aspect of the \texttt{Mapper} interface, namely its ability
to manage different formats of binary data.
...
\texttt{Message} structures for exchanging data between two endpoints.
