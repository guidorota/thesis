...
\texttt{Mapper} instances may be needed to interact with a single device, since
even the simplest I/O protocol usually requires several \texttt{Message}
structures for exchanging data between two endpoints.
...
means of the \texttt{getMessageType()} method. This is usually the first method
to be invoked when interacting with \texttt{Message} objects or raw data in
general, as it allows the Middleware to select the most appropriate component
for handling the information at hand.
...
This structural gap is bridged via the \texttt{marshal()} and
\texttt{unmarshal()} methods made available by the \texttt{Mapper} interface.
As can be seen by analyzing their respective signatures, these two methods
are used to convert a \texttt{Message} into a \texttt{Payload} and
vice-versa. This additional functionality introduces another aspect of the
\texttt{Mapper} interface, namely its ability to manage different formats of
binary data.
...
Creation of new mappers through the factory mechanism
