...
two endpoints, but is not suited to other forms of information management.
Handling data by the byte is unwieldy and unnecessarily complex, as it prevents
final users from leveraging the underlying structural characteristics of the
information to be processed. To overcome the limits imposed by the
\texttt{Payload} interface, an additional level of abstraction is required.
...
specific data item in the scope of a single \texttt{Message}. This attribute
can be used to address a particular field in order to change or retrieve its
value through the \texttt{setField()} and \texttt{getField()} methods.
...
\begin{itemize}
  ...
  using the \texttt{setField()} and \texttt{getField()}.
...
can utilize to expand the core functionalities of the PerLa Middleware. 
