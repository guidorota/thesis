...
instructions currently available in PerLa, is available in the remainder of
this section.
...
Basic usage, submits the ``start\_sampling'' \texttt{IORequest} to a
...
Submits the ``get\_data'' \texttt{IORequest} to a \texttt{HTTPChannel}. All
bytes received from the remote server are stored in the ``result'' variable.
...
Submits the ``send\_command'' \texttt{IORequest} to a \texttt{SerialChannel}.
The ``command'' variable is set as an \texttt{IORequest} parameter.
...
be used to set one record field. As soon as all the desired values are ready,
the content of the entired staging area can be flushed into a new record by
means of the \texttt{emit} instruction.
...
of struct unrolling procedures. The ability to retain field values set with
previous invocations of the \texttt{put} instruction is key to the example of
figure~\ref{fig:script_unrolling}, where low frequency data --- namely the
\textit{name-id} and the \textit{city} records --- is set only once, and only
the high-frequency information is constantly overwritten with new calls to the
...
All records emitted are sent to output as soon as the script hits a stop
instruction
...
If..then example. Sets the variable ``alarm'' to TRUE when the temperature
...
If..then..else example. Sets the variable ``tropical'' to TRUE when the
...
Emits a new record for each list item.
