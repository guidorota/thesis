...
\texttt{lst:jsonmessage} provide a great opportunity for delving into the
technique employed by the PerLa Middleware to map an XML fragment into a
specific \texttt{MessageDescriptor} class. Each XML element is prefixed by an
appropriate namespace identifier, which is used by the \texttt{JAXB} library to
identify onto which descriptor class the current XML snippet has to be
deserialized. In our examples, \textit{js} stands for
\texttt{JSONMessageDescriptor}, whereas \textit{ue} corresponds to
\texttt{URLEncodedMessageDescriptor}.
