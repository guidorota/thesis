...
with the burden of managing data in byte-sized chunks.

The specific structure of a \texttt{Message} is defined by its data type.  This
property unequivocally identifies the set of fields contained in a
\texttt{Message} in terms of field \textbf{name}, field \textbf{type} and field
\textbf{qualifier}.
...
to address a particular field in order to change or retrieve its value.
...
The PerLa Middleware currently supports the following primitive data types:
\begin{itemize}
...
nested \texttt{Message}. In this case, the field type attribute must be set to
the particular type of \texttt{Message} that has to be stored. 

The \textbf{qualifier} attributes specifies additional properties of the field.
