...
through a \texttt{MapperFactory} calls for two additional parameters other than
the descriptor: a \texttt{ClassPool}, and a map of \texttt{Mapper}s. These
extra items provide a reference to previously built \texttt{Mapper}s, and can
be used to check whether nested \texttt{Message} fields were properly declared
or not.
...
implementations, each of which is dedicated to a single data format. JSON and
URL-Encoded \texttt{Mapper}s are therefore created by two distinct
\texttt{MapperFactory} objects, namely \texttt{JSONMapperFactory} and
\texttt{URLEncodedMapperFactory}. Moreover, several 
