...
registered in a PerLa instance, and its services are extensively exploited by
the Query Executor component. Its interface, available in
listing~\ref{lst:registry}, is straightforward and easy to use. It is composed
of two data manipulation methods, namely \texttt{add()} and \texttt{remove()},
and three data retrieval methods, \texttt{get()}, \texttt{getAll()} and
\texttt{getByAttribute}.
...
single \texttt{FPC} object with a specific identifier; it is usually invoked
when the user needs to address a certain device in a Pervasive Network.
\texttt{getAll()} is a shortcut method that returns a list with all the
\texttt{FPC} objects connected to PerLa. The last method,
\texttt{getByAttribute()}, allows PerLa users to select a subset of devices
with well-defined characteristics; its two parameters can in fact be used to
indicate the precise set of data \texttt{Attributes} that the remote device
must possess in order to be considered in the selection.
