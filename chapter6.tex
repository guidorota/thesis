...
even the simplest I/O protocols usually requires the use of several
...
general, as it allows the Middleware to select the most appropriate tool for
handling the information at hand.

Once a \texttt{Mapper} is selected, new \texttt{Message} instances can be
created through the \texttt{createMessage()} method. Every \texttt{Message}
instantiated using this technique acquires the data type of its parent
\texttt{Mapper}. This one-to-one relationship between \texttt{Mapper}s and
\texttt{Message} types is further epitomized by the
...
The \texttt{getFieldDescriptors()} method holds a pivotal role in the
implementation of the type safety mechanisms available in the Middleware, as
the possibility to query the type of every field in a \texttt{Message} can be
used to check wether a specific operation can be performed safely or not.

As explained in the previous section, \texttt{Message} objects are a
convenience introduced for simplifying data management operations in the PerLa
Middleware. They provide structured access to information, a familiar set of
primitive data types, and a selection of tools for combining basic values into
complex data structures. In spite of these advantages, the \texttt{Message}
interface is a high level abstraction that cannot be employed where a
\texttt{Payload} is expected, since it doesn't provide any method for accessing
its contents as a simple sequence of bytes.  As a consequence,
\texttt{Message}s can't be used directly in any kind of I/O operation.
...
Every \texttt{Mapper} created in the PerLa Middleware committed to the
...
Each \texttt{Mapper} object created in the PerLa Middleware is committed to the
