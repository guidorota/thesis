...
of the Middleware will contain a wide variety of potentially custom
\texttt{MapperFactory} implementations, each of which is dedicated to a single
data format. Instances of the previously introduced JSON and URL-Encoded
mappers, for example, are created by two distinct \texttt{MapperFactory}
objects, namely \texttt{JSONMapperFactory} and
\texttt{URLEncodedMapperFactory}. This design provides an improved separation
of concerns with respect to the previous middleware architecture, since it
ensures that every \texttt{MapperFactory} is responsible for managing the
quirks of only a single data format.

Moreover, every \texttt{MapperFactory} implementation is bundled with a custom
\texttt{MessageDescriptor} object, whose class is exposed by the aforementioned
\texttt{acceptedMessageDescriptorClass()} method. The additional complexity
deriving from this design choice is more than made up for in type safety and
flexibility, as each different \texttt{MessageDescriptor} may be used to
enforce restrictions or to allow greater freedom of expression while declaring
new message types. The \texttt{URLEncodedMessageDescriptor} class, for example,
cannot be used to declare a \texttt{Message} within another \texttt{Message},
as its fields can only host primitive data types
(listing~\ref{lst:urlencodedmessage}); \texttt{JSONMessageDescriptor}, on the
other hand, doesn't impose such restriction, since the JSON data format allows
the possibility of nesting one object into the other
(listing~\ref{lst:jsonmessage}).
...
into the technique employed to convert an XML fragment in a specific Java
object. The namespace affixed to all elements of the descriptor is used by the
\texttt{JAXB} library to identify which message descriptor class the current
XML snippet has to be deserialized into. In our examples, \textit{js} stands
for \texttt{JSONMessageDescriptor}, whereas \textit{ue} corresponds to
\texttt{URLEncodedMessageDescriptor}. This deserialization method has an
important advantage over a universal message descriptor textual format, that is
the possibility of using a custom set of XML elements for each distinct
