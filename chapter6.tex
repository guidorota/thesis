...
\subsection{The Message interfaces}
...
\begin{itemize}
  ...
  exception to be thrown. It is important to note that the static field
  values are set on a per-type basis; this means that all \texttt{Message}s of
  the same type will share the same field values for each static field (if
  any). 
\end{itemize}
