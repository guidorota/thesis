...
of note regarding the \texttt{OperationScheduler} is its ability simulate a
periodic sampling activity by executing a single-shot \texttt{Get Operation}
repeatedly.
...
Figure~\ref{fig:fcpfactory} shows a section of a typical Middleware setup. As
can be seen, Device Descriptors are parsed and transliterated into an
intermediate Java representation before being handed over to the
...
Upon arrival of a new Device Descriptor

The device descriptor in XML is parsed and converted into java objects, this
gives us the possibility to change the descriptor syntax without having to
scrap the fpcfactory
the fpc factory then takes all the device descriptor parts and sends them to
the most appropriate component factory (e.g., channel to channel factory, etc).
It uses Java reflection and the accepted*descriptorclass method to identify
which sub-factory to choose for creating the component.
Moreover, it parses the operations, and invokes the script compiler to parse
the SIR.
Once all these operations are done, it takes all the sub-components and
assembles a new fpc
