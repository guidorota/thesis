...
single type of \texttt{Message}. Its interface, available in
listing~\ref{lst:mapper}, provides a series of functionalities that are crucial
for governing the entire lifecycle of a \texttt{Message} object. 
...
means of the \texttt{getMessageType()} method. This method can be used to
distinguish between the multiple \texttt{Mapper} instances that may be needed
to interact with a single device, since even the simplest I/O protocol usually
requires several \texttt{Message} structures for exchanging data between two
endpoints. 

As even the simplest I/O protocol usually
requires several \texttt{Message} structures for exchanging data between two
endpoints, the Middleware


This is generally the first method to be
invoked when interacting with \texttt{Message} objects or raw data in general,
as it is used by the Middleware to select the most appropriate component for
handling the information at hand.

Multiple \texttt{Mapper} instances may be needed to interact with a single
device, since even the simplest I/O protocol usually requires several
\texttt{Message} structures for exchanging data between two endpoints. The type
of \texttt{Message} managed by a \texttt{Mapper} can be queried by means of the
\texttt{getMessageType()} method. This is generally the first method to be
invoked when interacting with \texttt{Message} objects or raw data in general,
as it is used by the Middleware to select the most appropriate component for
handling the information at hand.

Once the desired \texttt{Mapper} is chosen, new \texttt{Message} instances can
be created through the \texttt{createMessage()} method. Every \texttt{Message}
instantiated using this technique acquires the data type of its parent
\texttt{Mapper}, along with its related field structure. The one-to-one
relationship between \texttt{Mapper}s and \texttt{Message} types is further
epitomized by the \texttt{getFieldDescriptors()} method, which returns a
complete description of every field included in a specific \texttt{Message}
type. This method holds a pivotal role in the implementation of the
Middleware's type safety mechanism, where the possibility to introspect the
internal structure of a \texttt{Message} object is heavily exploited for
checking wether a specific operation can be safely performed or not.
...
can't be used in any kind of I/O operation.

This structural gap is bridged by the \texttt{Mapper} interface via its
\texttt{marshal()} and \texttt{unmarshal()} methods. As can be seen by
analyzing their respective signatures, these two methods perform a conversion
process that can be used to transform \texttt{Message} objects into a
\texttt{Payload} and vice-versa. This additional functionality introduces
another aspect of the \texttt{Mapper} interface, namely its ability to manage
different formats of binary data.
...
Every \texttt{Mapper} class is in fact designed to support a single data
format. \texttt{JSONMapper} instances, for example, handle JSON-formatted byte
streams, whereas \texttt{URLEncodedMapper}s specialize in the conversion of
URL-encoded HTTP entities. The structure of the \texttt{Message}s created by a
...
URL-encoded format, for example, does not provide any support for nested data
structure; hence, the corresponding \texttt{URLEncodedMapper} class is designed
to disallow all \texttt{Message} types that contains non-primitive fields.

The additional decoupling provided by the \texttt{Mapper} interface builds over
the aforementioned pluggable \texttt{Channel} design, thus allowing
\texttt{Mapper}s and \texttt{Channel}s to be selected independently of each
other (see figure~\ref{fig:mapper_channel}).
