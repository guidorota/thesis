...
single type of \texttt{Message} object. Multiple \texttt{Mapper} instances are
usually required to interact with a single device, as even the simplest I/O
protocol may require several \texttt{Message}s to be exchanged between two
...
commonly invoked when interacting with \texttt{Message} objects, as it allows
the Middleware to select the appropriate tool to handle the data at
hand.

Once a \texttt{Mapper} is selected, new \texttt{Message} instances can be
created through the \texttt{createMessage()} method. Every \textt{Message}
instantiated using this technique will inherit the data type managed by the
\textt{Mapper} from which it was created. This one-to-one relationship between
