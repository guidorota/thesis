...
of addressing information by field name, thus releasing the final user from the
burden of managing data in byte-sized chunks.

The specific composition of a \texttt{Message} is defined by its data type.
This property unequivocally identifies the set of fields contained in a
\texttt{Message} in terms of field name, type and qualifier.

The field \textbf{name} is a textual string that uniquely identifies a specific
field in the scope of a single \texttt{Message}. This attribute can be used to
address a particular field in order to change or retrieve its value.

The \textbf{type} attribute defines the set of legal values that can be assumed
by the field, together with the operations can be performed on those values.
The PerLa Middleware currently supports the following data types:
\begin{itemize}
  ...
  \item \textbf{TIMESTAMP}: a date, currently implemented using Java's
  \texttt{ZonedDateTime} class
  \item \textbf{ID}: a unique
\end{itemize}

The \textit{qualifier} attributes specifies additional properties of the field.
The PerLa Middleware currently supports three qualifiers, namely SIMPLE, STATIC
and LIST, whose meaning will be explained later in this section.
