...
potentially similar XML tags and attributes are avoided through the
...
attributes. PerLa \texttt{Scripts} currently support two different types of
...
expression \lstinline!${original}! expression.
...
\texttt{error} is commonly guarded by an \texttt{if} control structure to
implement advanced error management behaviours.
...
depending on whether the \textbf{condition} expression evaluates to true or
false.
