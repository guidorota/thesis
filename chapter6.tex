...
\end{itemize}
Besides the data types presented above, a field can also hold a nested
\texttt{Message}. In this case, the type attribute must be set to the
...
\textbf{marshalling} when a structured \texttt{Message} is encoded into a
\texttt{Payload}, and \textbf{unmarshalling} when a byte sequence enclosed in a
...
streams, whereas the \texttt{URLEncodedMapper}s specialize in the conversion of
...
independently of each other (see figure~\ref{fig:mapper_channel}).

As shown in listing~\ref{lst:mapper}, the \texttt{Mapper} interface goes well
...
lifecycle of a \texttt{Message} object. Each \texttt{Mapper} created in the
PerLa Middleware is in fact committed to the administration of a specific
\texttt{Message} type, whose identifier can be retrieved via the
\texttt{getMessageType()} method.

As exemplified in figure~\ref{fig:mapper.messages}, multiple instances of the
same \texttt{Mapper} class are present in a single PerLa installation, each one
of which is responsible for the management of a different type of
\texttt{Message}. 
