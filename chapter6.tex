...
and vice-versa. This additional functionality brings to light yet another
aspect of the \texttt{Mapper} layer, namely its ability to manage different
...
structures, as it is defined as a flat collection of key-value pairs; hence,
the corresponding \texttt{URLEncodedMapper} class could never be used to create
and manage \texttt{Message}s with nested fields. The close connection between a
...
occupied by the \texttt{Mapper} interface, relatively to the \texttt{Channel}

The additional decoupling provided by the \texttt{Mapper} interface builds over
the pluggable \texttt{Channel} design described in the previous section, thus
allowing \texttt{Mapper}s and \texttt{Channel}s to be selected independently of
each other (see figure~\ref{fig:mapper_channel}). This is an important
characteristic of the Middleware design, as even the simplest I/O protocol
usually requires several \texttt{Message} structures for exchanging data
